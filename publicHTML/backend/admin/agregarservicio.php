...
function crearServicio($pdo) {
    ...
    if (($iconoName && $imagenName) || ($iconoName && $imagenName == null) || ($iconoName == null && $imagenName)) {
        ...
        if ($stmt->execute()) {
            $respuesta["exito"] = "Se ha agregado el servicio y ahora es visible para los pacientes";
        } else {
            $respuesta["error"] = "Ha ocurrido un error al agregar el servicio";
        }
    } else {
...
function icono($icon) {
    ...
    if($icon != null){
        ...
        try {
            if (move_uploaded_file($icon['tmp_name'], $ruta_guardar_archivo)) {
...
function imagen($img) {
    ...
    if($img != null){
        ...
        try {
            if (move_uploaded_file($img['tmp_name'], $ruta_guardar_archivo)) {
                return $nuevo_nombre_archivo;
            } else {
                return false;
            }
